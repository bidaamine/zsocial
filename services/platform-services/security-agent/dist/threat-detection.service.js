...
let ThreatDetectionService = ThreatDetectionService_1 = class ThreatDetectionService {
    ...
    async assessRisk(ip, action, userId) {
        ...
        if (lastHistory && lastHistory.ip !== ip) {
            const distance = this.calculateDistance(lastHistory.lat, lastHistory.lon, currentCoords.lat, currentCoords.lon);
